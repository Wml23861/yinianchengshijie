...
- **事件讨论 + 雪花法逐层展开** — `/novel-discuss` 事件画布用雪花法逐层放大：一句话方向 → 因果结构 → 章节拆分 → 剧情链，前三层各设确认门、逐层落盘到事件卡草稿（可断点续接、可局部修订），第四层剧情链由 AI 推导；剧情链直接作为 novel-write 的写作输入，事件卡未走到 `done` 时 novel-write 会拦截
- **正文创作** — `/novel-write` 从事件卡读剧情链 → 强制反思 → 写作 → 后处理（一致性校验 + AI 味检测 + 润色）
...
| `/novel-write`     | 写章节、创作正文       | 从事件卡读剧情链 → 强制反思（四维定调 + 预判翻车点 + 核心瞬间 + 禁用句式）→ 写作 → 后处理（一致性校验 + AI 味检测 10 项 + 独立润色）；交付后提示用户运行 `/novel-revise` 返修或 `/novel-sync` 同步 |
...
| `/novel-obsidian`  | Obsidian、可视化       | 同步实体关系到 Obsidian vault，利用图谱视图可视化关系网络                                                  |
...
/novel-bookplan      →  全书架构与卷规划（可选）
    ↓
/novel-discuss（事件画布）→  L1 方向门 → L2 结构门 → L3 章节门 → L4 剧情链推导（AI 主导）→ 逐层落盘事件卡
    ↓
/novel-write         →  从事件卡读剧情链 → 强制反思 → 写作 → 后处理
    ↓
生成最终稿           →  后处理通过后正文落盘
    ↓
...
这套工作流现在分成 6 个阶段，核心原则是：设定先落长期记忆，结构先落 `future/`，事件用雪花法逐层展开（方向/结构/章节三道确认门，剧情链由 AI 推导、不单独确认），直接进入正文创作。`booming` 是插在剧情讨论阶段里的可选爆破分支，`fuck-it` 是插在剧情讨论阶段里的同终点加戏分支，它们都不是默认主流程节点。
...
- 这一步服务于 future/ 输入，不替代 `/novel-discuss` 的章节级剧情链
...
`/novel-write` 从事件卡中读取本章的剧情链，而不是自己重做结构。

流程：
1. 读事件卡，提取本章剧情链
2. **写作前强制反思**（thinking 中完成，不输出）：四维定调 → 预判翻车点 → 核心瞬间 → 禁用句式
3. 从剧情链直接展开为具象正文，以反思结论为内置约束
...
所以现在的设计是：结构收敛发生在 `novel-discuss` 的事件画布阶段（产出剧情链）；`novel-write` 从事件卡读剧情链，做强制反思从源头防 AI 腔，再把剧情链展开为血肉，交付后由用户决定是否需要返修。
...
回事件讨论修订剧情链 → 再试写 → `/novel-write` → `/novel-revise` → `/novel-sync`
...
```bash
# 快速检查核心文案是否保持一致
rg -n "事件定位|故事大纲|剧情链|方向门|结构门|章节门|设计阶段|草稿防护|四通道|fuck it|fuck-it|本章结束目标|AI 味检测|强制反思" \
  README.md \
...
- `docs/plans/2026-04-07-outline-format-redesign*.md`：记录已废弃 `novel-plan` 从旧版章节大纲格式转为第三人称精简剧情纲要的设计与实施（功能已并入 `novel-discuss`）。
- `docs/plans/2026-04-28-novel-plan-story-outline-design.md`：记录已废弃 `novel-plan` 从 SSoT 发散 + 5W1H 填表改为故事大纲 + 逐步澄清流程的设计（功能已并入 `novel-discuss`）。
- `docs/plans/2026-04-29-novel-plan-narrative-outline.md`：记录已废弃 `novel-plan` 故事梗概落盘、场景纲要改为叙述式段落、新增大纲 AI 味检测的设计（功能已并入 `novel-discuss`）。
...
- `docs/plans/2026-05-29-event-canvas-snowflake-design.md`：记录事件画布改为雪花法四层逐层展开（方向/结构/章节三道确认门 + 逐层落盘草稿 + `设计阶段` 状态字段 + 修订回退 + novel-write 草稿防护）的设计，含"旧两项确认边界 → 新三道门"的契约迁移 rationale。
